...
| M18: 数据解析修复 | ✅ 已完成 | 2026-02-05 | 修复心率和血糖数据不显示问题 |
| M19: 用户使用手册 | ✅ 已完成 | 2026-02-05 | 编写详细的用户使用手册 |
...
| M27: 首页真实数据集成 | ✅ 已完成 | 2026-02-06 | 修复控制器依赖+首页真实数据显示 |
| M28: Release版本显示修复 | ✅ 已完成 | 2026-02-09 | 创建ID=1测试用户解决Release版无法显示家庭成员 |
| M29: 健康提醒功能 | ✅ 已完成 | 2026-02-12 | 使用定时器每分钟检查实现提醒，真机测试通过 |
| M30: 首页成员头像UI修复 | ✅ 已完成 | 2026-02-13 | 修复首页成员名字被遮挡问题，优化布局 |
| M31: 血压/血糖趋势图表功能 | ✅ 已完成 | 2026-02-13 | 血压双折线图、血糖正常范围、趋势分析页面 |
| M32: 健康统计页面数据验证重构 | ✅ 已完成 | 2026-02-14 | 创建DataValidator工具类，修复FormatException |
| M33: 健康趋势页面成员筛选 | ✅ 已完成 | 2026-02-14 | 新增成员选择器，按成员筛选统计数据和图表 |
...
| 时间 | 问题 | 解决方案 |
|------|------|----------|
| 2026-02-09 | Release版无法显示家庭成员 | 数据库创建ID=1测试用户 |
...
**问题描述**：
- Debug版本（连接localhost）家庭成员显示正常
- Release版本（连接远程服务器）无法显示家庭成员
- 服务器日志显示：`您还未加入家庭`
...
| 阶段 | 操作 | 发现 |
|------|------|------|
| 1. 前端排查 | 检查JwtAuthenticationFilter、FamilyController | 认证逻辑正常 |
| 2. 后端部署 | SSH服务器，上传修改后代码，编译重启 | 服务正常运行 |
| 3. 日志分析 | 查看服务器日志 | userId=1，但找不到该用户 |
| 4. 数据库查询 | 查询user表 | 用户ID是雪花算法大整数 |

**根因定位**：
- Flutter应用存储的userId是`"1"`（测试数据）
- 数据库用户ID是雪花算法生成的超大值（如`2019307347694460930`）
- 后端找不到ID为1的用户，返回`您还未加入家庭`
...
**问题诊断**：
- ✅ family记录的`deleted`字段为1（逻辑删除状态）
- ✅ user的`family_role`为`member`而非`admin`
- ✅ Windows curl发送中文时UTF-8编码异常
...
**新增文件**：
- ✅ `dto/FamilyUpdateNameRequest.java` - 更新家庭名称DTO

**新增接口**：
| 接口 | 方法 | 说明 | 状态 |
|------|------|------|------|
| /api/family/name | PUT | 更新家庭名称（RequestBody） | ✅ 正常 |
| /api/family/update-name | POST | 更新家庭名称（RequestParam） | ✅ 正常 |
...
**数据库变更**：
- ✅ 创建数据库迁移脚本 `migration-family.sql`
  - 新增 `family` 表（家庭ID、家庭名称、邀请码、管理员、成员数）
  - 修改 `sys_user` 表添加 `family_id` 和 `family_role` 字段
  - 修改 `family_member` 表添加 `family_id` 字段
  - 修改 `health_data` 表添加 `family_id` 字段

**后端实现**（11个文件）：
- ✅ `entity/Family.java` - 家庭实体类
- ✅ `mapper/FamilyMapper.java` - 家庭Mapper
- ✅ `dto/FamilyResponse.java` - 家庭响应DTO
- ✅ `dto/FamilyCreateRequest.java` - 创建家庭请求DTO
- ✅ `dto/FamilyQrCodeResponse.java` - 二维码响应DTO
- ✅ `dto/FamilyJoinRequest.java` - 加入家庭请求DTO
- ✅ `dto/FamilyMemberUserResponse.java` - 家庭用户响应DTO
- ✅ `service/FamilyService.java` - 家庭服务接口
- ✅ `service/impl/FamilyServiceImpl.java` - 家庭服务实现（生成6位邀请码）
- ✅ `controller/FamilyController.java` - 家庭控制器（9个API接口）
- ✅ 更新 `User.java`、`UserVO.java`、`AuthResponse.java` 添加家庭字段

**API接口**（9个）：
| 接口 | 方法 | 说明 |
|------|------|------|
| /api/family/create | POST | 创建家庭 |
| /api/family/my | GET | 获取我的家庭 |
| /api/family/qrcode | GET | 获取家庭二维码 |
| /api/family/info/{code} | GET | 解析邀请码 |
| /api/family/join | POST | 通过邀请码加入 |
| /api/family/leave | POST | 退出家庭 |
| /api/family/members | GET | 获取家庭成员列表 |
| /api/family/members/{userId} | DELETE | 移除成员 |
| /api/family/name | PUT | 更新家庭名称 |

**前端实现**（7个文件）：
- ✅ `lib/core/models/family.dart` - 家庭数据模型（Family、FamilyQrCode、FamilyUser）
- ✅ `lib/core/models/user.dart` - 更新User模型添加familyId/familyRole字段
- ✅ `lib/app/modules/family/family_controller.dart` - 家庭控制器
- ✅ `lib/app/modules/family/family_binding.dart` - 依赖注入
- ✅ `lib/app/modules/family/family_create_page.dart` - 创建家庭页面
- ✅ `lib/app/modules/family/family_qrcode_page.dart` - 二维码展示页面
- ✅ `lib/app/modules/family/family_scan_page.dart` - 扫码加入页面
- ✅ `lib/app/modules/family/family_members_page.dart` - 家庭成员列表页面
- ✅ 添加 `qr_flutter: ^4.1.0` 依赖
- ✅ 更新路由配置添加家庭相关路由
- ✅ 在个人中心页面添加家庭管理入口

**功能特性**：
1. **创建家庭**：用户可创建家庭，自动成为管理员，生成6位唯一邀请码
2. **生成二维码**：管理员可查看家庭二维码（内容：FAMILY_INVITE:ABC123）
3. **扫码加入**：使用mobile_scanner扫描二维码或手动输入邀请码
4. **成员管理**：查看家庭成员列表，管理员可移除成员
5. **退出家庭**：普通成员可退出家庭
6. **数据共享**：健康数据在家庭内共享（通过family_id关联）

**编译验证**：
- ✅ Flutter pub get成功（qr_flutter已安装）
- ✅ 路由配置正确

### 2026-02-06 晚（角色权限控制功能完成）

**用户使用手册**：
- ✅ 创建 `docs/user-manual.md` 详细用户手册
- ✅ 约15000字，14个主要章节
- ✅ 包含完整的功能介绍和操作指南
- ✅ 15个常见问题解答

**数据解析问题修复**：
- ✅ 修复 `_parseDataType` 方法数据类型映射问题
- ✅ 支持后端 snake_case (heart_rate) 到前端 camelCase (heartRate) 的转换
- ✅ 手机验证通过，心率和血糖数据正常显示
